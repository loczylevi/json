...
while harcos1.alive == True:
    ...
    sebzes_levonas = harcos2.take_damage(harcos2, vedekezes_levonas)
    print(sebzes_levonas)
    #print(type(gayming))
    gayming["characters"][1]['health'] = sebzes_levonas
    print(gayming["characters"][1]['health'])
    time.sleep(1)
    with open("gaming.json", "w", encoding="utf-8") as f:
    ...
    if gayming["characters"][1]["health"] <= 0 or gayming["characters"][0]["health"] <= 0:
        print("meghalt")
        break
    
    #______________________________vissza ütés_______________________________________-
    print("#______________________________vissza ütés________________________________________")
    print(f"'{harcos2.nev}' támadja --> '{harcos1.nev}' ")
    print(f"{harcos2.nev}: attack: {harcos2.tamadoero} HP: {harcos2.eletero}")
    print(f"{harcos1.nev}: attack: {harcos1.tamadoero} HP: {harcos1.eletero}\n___________________________________________________________________")
    vedekezes_levonas2 = harcos2.harc(harcos2, harcos1)
    print(vedekezes_levonas2)
    print("______________________________________________________________")
    sebzes_levonas2 = harcos2.take_damage(harcos1, vedekezes_levonas)
    print(sebzes_levonas2)
    gayming["characters"][0]['health'] = sebzes_levonas2
    time.sleep(1)
    print(f'HP_ {gayming["characters"][0]["health"]}')
    with open("gaming.json", "w", encoding="utf-8") as f:
        f.write(json.dumps(gayming, indent=4))
